...
GLOBAL_GROOVE_AMOUNT_OFFSET = 0x02
GLOBAL_GROOVE_TYPE_OFFSET = 0x03
GLOBAL_CLICK_VOLUME_OFFSET = 0x04
GLOBAL_ACTIVE_SCENE_OFFSET = 0x06
GLOBAL_ACTIVE_SONG_OFFSET = 0x07
...
class ProjectConfig:
    ...
    groove_type_raw: int
    groove_amount_raw: int
    click_volume_raw: int
    ...
    @property
    def groove_amount(self) -> int:
        return decode_signed_i8(self.groove_amount_raw)

    @property
    def metronome_enabled(self) -> bool:
        """Best-effort persisted metronome state.

        Firmware 1.1.4 captures store metronome off identically to click
        volume minimum: both set the click-volume byte to 0. No independent
        on/off bit moved in the isolated probes.
        """
        return self.click_volume_raw != 0
...
def decode_signed_i8(raw: int) -> int:
    if raw >= 0x80:
        return raw - 0x100
    return raw
...
def decode_transpose(raw: int) -> int:
    ...
    return decode_signed_i8(raw)


def encode_signed_i8(value: int) -> int:
    if not -128 <= value <= 127:
        raise ValueError("signed i8 value must be in -128..+127")
    return value & 0xFF
...
def encode_groove_amount(amount: int) -> int:
    return encode_signed_i8(amount)
...
def read_project_config(project: ImageProject) -> ProjectConfig:
    ...
    return ProjectConfig(
        ...
        groove_amount_raw=image[GLOBAL_GROOVE_AMOUNT_OFFSET],
        click_volume_raw=image[GLOBAL_CLICK_VOLUME_OFFSET],
        active_scene_raw=image[GLOBAL_ACTIVE_SCENE_OFFSET],
        active_song_raw=image[GLOBAL_ACTIVE_SONG_OFFSET],
